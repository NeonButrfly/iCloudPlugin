...
- leave `ICLOUD_MIRROR_ROOT=/srv/cloud-vault/mirrors` so the container path
  stays stable
- keep the `service` and `worker` container binds for `/srv/cloud-vault`
  writable, not `:ro`, because the origin API and the background worker now
  both own live `document-vault` mutations such as queued note creation and
  note reindex work
