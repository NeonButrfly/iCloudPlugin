...
export const TOOL_SUBMISSION_DETAILS = {
  ...
  get_icloud_change_set: {
    ...
    justifications: {
      read_only_justification:
        "Only retrieves indexed metadata and item history for a previously recorded reversible change set.",
      open_world_justification:
        "Does not publish or mutate public internet state or third-party systems.",
      destructive_justification:
        "Does not delete, overwrite, or perform irreversible actions.",
  ...
  get_cloud_vault_task_status: {
    ...
    justifications: {
      read_only_justification:
        "Only retrieves persisted task status, progress, and result metadata without mutating backend state.",
      open_world_justification:
        "Does not publish or mutate public internet state or third-party systems.",
      destructive_justification:
        "Does not delete, overwrite, or perform irreversible actions.",
  ...
  list_cloud_vault_tasks: {
    ...
    justifications: {
      read_only_justification:
        "Only lists persisted cloud-vault task records and their current statuses without changing backend state.",
      open_world_justification:
        "Does not publish or mutate public internet state or third-party systems.",
      destructive_justification:
        "Does not delete, overwrite, or perform irreversible actions.",
  ...
  get_icloud_dedupe_group: {
    ...
    justifications: {
      read_only_justification:
        "Only retrieves indexed metadata and member items for a duplicate-group proposal.",
      open_world_justification:
        "Does not publish or mutate public internet state or third-party systems.",
      destructive_justification:
        "Does not delete, overwrite, or perform irreversible actions.",
  ...
  continue_cloud_vault_task: {
    ...
    justifications: {
      read_only_justification:
        "Advances a persisted cloud-vault task by one bounded step, so it changes internal backend task state.",
      open_world_justification:
        "Only mutates the private cloud-vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Does not hard delete data or perform irreversible actions by itself; it only advances queued private workflows.",
    },
  },
  continue_cloud_vault_task_queue: {
    ...
    justifications: {
      read_only_justification:
        "Advances a bounded batch of queued private tasks, so it changes internal backend task state.",
      open_world_justification:
        "Only mutates the private cloud-vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Does not hard delete data or perform irreversible actions by itself; it only advances queued private workflows.",
  ...
  cancel_cloud_vault_task: {
    ...
    justifications: {
      read_only_justification:
        "Changes internal backend state by canceling a queued or running private task before it completes.",
      open_world_justification:
        "Only mutates the private cloud-vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Cancels future execution but does not hard delete data or perform irreversible file removal.",
    },
  },
  queue_create_document_vault_note_from_file_id_chatgpt_first: {
    ...
    justifications: {
      read_only_justification:
        "Queues file-id-based note creation work and may later create a private document_vault note, so it changes internal backend task state.",
      open_world_justification:
        "Only uses private cloud-vault backend services and does not publish or transmit changes to public internet systems.",
      destructive_justification:
        "Does not hard delete source data; it queues a reversible note-side workflow with optional fallback behavior.",
    },
  },
  queue_create_document_vault_notes_from_search: {
    ...
    justifications: {
      read_only_justification:
        "Queues note-creation work for a bounded search result set, so it changes internal backend task state.",
      open_world_justification:
        "Only uses private cloud-vault backend services and does not publish or transmit changes to public internet systems.",
      destructive_justification:
        "Does not hard delete source data; it queues a reversible note-side workflow for explicit matching files only.",
    },
  },
  queue_classifier_fallback_note_from_file_id: {
    ...
    justifications: {
      read_only_justification:
        "Queues fallback-only local-classifier note creation for one explicit file id, so it changes internal backend task state.",
      open_world_justification:
        "Only uses private cloud-vault backend services and does not publish or transmit changes to public internet systems.",
      destructive_justification:
  ...
  sync_icloud_manual_feedback_events: {
    ...
    justifications: {
      read_only_justification:
        "Persists indexed manual-feedback event records from private document_vault notes, so it changes internal backend state.",
      open_world_justification:
        "Only mutates the private synced vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Does not delete or overwrite source files; it records reversible metadata about manual feedback signals.",
    },
  },
  ...
  queue_dedupe_analysis: {
    ...
    justifications: {
      read_only_justification:
        "Queues resumable dedupe analysis work, so it changes internal backend task state.",
      open_world_justification:
        "Only mutates the private synced vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Does not delete or overwrite files; it queues bounded duplicate-analysis work for later review.",
    },
  },
  ...
  queue_apply_icloud_dedupe_group: {
    ...
    justifications: {
      read_only_justification:
        "Queues non-destructive dedupe application through reversible backup storage, so it changes internal backend task state.",
      open_world_justification:
        "Only mutates the private synced vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Does not hard delete files; it queues approved duplicate moves through reversible backup storage.",
    },
  },
  queue_restore_icloud_change_set: {
    ...
    justifications: {
      read_only_justification:
        "Queues a reversible change-set restore on the private backend, so it changes internal backend task state.",
      open_world_justification:
        "Only mutates the private synced vault backend and does not publish or send changes to public internet systems.",
      destructive_justification:
        "Restores previously backed-up files rather than hard deleting data or performing irreversible actions.",
...
export const TEST_CASES = [
  ...
  {
    description: "Inspect one reversible change set and its indexed items.",
    user_prompt: "Show me the indexed history for change set abc123.",
    file_attachment_urls: null,
    tools_triggered: "get_icloud_change_set",
    expected_output:
      "Returns the change-set metadata and indexed item rows for the requested reversible operation.",
    expected_output_url: null,
  },
  {
    description: "Inspect one duplicate-group proposal and its members.",
    user_prompt: "Show me the indexed duplicate-group proposal dup123.",
    file_attachment_urls: null,
    tools_triggered: "get_icloud_dedupe_group",
    expected_output:
      "Returns the duplicate-group metadata and member rows for the requested proposal.",
    expected_output_url: null,
  },
  {
  ...
  {
    description: "Persist manual-feedback events from the live document vault.",
    user_prompt: "Re-read the Obsidian vault and persist the latest manual feedback events.",
    file_attachment_urls: null,
    tools_triggered: "sync_icloud_manual_feedback_events",
    expected_output:
      "Returns a count of scanned and newly created indexed manual-feedback events.",
    expected_output_url: null,
  },
  {
    description: "Analyze live mirrored files for duplicate candidates.",
    user_prompt: "Analyze google1, google2, and icloud for duplicates and persist candidate groups.",
    file_attachment_urls: null,
    tools_triggered: "analyze_icloud_duplicates",
    expected_output:
      "Returns created duplicate-group ids and candidate member sets without deleting or moving files.",
    expected_output_url: null,
  },
];
