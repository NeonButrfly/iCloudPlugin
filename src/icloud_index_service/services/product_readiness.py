...
def collect_repo_surface_facts(repo_root: Path) -> dict[str, Any]:
    ...
    github_secret_bootstrap_helper = (
        worker_dir / "scripts" / "bootstrap-github-secrets.mjs"
    )
    ...
    return {
        ...
        "remote_mcp_github_secret_bootstrap_helper_present": github_secret_bootstrap_helper.is_file(),
...
def _evaluate_auth_and_deploy_story(
    ...
    if repo_facts.get("remote_mcp_github_secret_bootstrap_helper_present") is not True:
        return ReadinessCheck(
            "blocked",
    ...
    if cloudflare_api_token_present:
        return ReadinessCheck(
            ...
            "Cloudflare deploy auth is present, but hosted proof still needs to be run.",
            {
                ...
                "github_secret_bootstrap_helper_present": True,
            },
        )
